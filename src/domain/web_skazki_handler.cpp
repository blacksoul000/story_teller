...
{
    QRegExp regexp("<title>(Аудио\\s?сказк[иа]|Аудио\\s?книга|Песни)\\s«([А-ЯЁа-яё0-9:,\\/\\.\\s\\-–_\\(\\)\\?!]+)» слушать онлайн<\\/title>");
...
{
    QRegExp regexp("src=\"(https:\\/\\/web-skazki\\.ru\\/preview-files\\/[a-zA-Z0-9\\-\\\\/\\]+\\-(x-[0-9]+|big)\\.(jpe?g|png))\"");
    regexp.setMinimal(true);
    if (regexp.indexIn(buffer) != -1)
    {
        return regexp.cap(1);
